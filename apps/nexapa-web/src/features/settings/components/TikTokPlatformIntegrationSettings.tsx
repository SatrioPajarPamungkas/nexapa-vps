...
import { TIKTOK_ENVIRONMENTS } from "../settings.constants";
...
export function TikTokPlatformIntegrationSettings({}: Props) {
  const {
    tiktokSettings,
    isLoading,
    isSaving,
  ...
  return (
    <div className="space-y-6 bg-transparent">
      ...
      <div className="mb-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-[14px] font-semibold text-slate-900">TikTok API Credentials</h3>
          <p className="mt-1 text-[12px] text-slate-600">Configure your TikTok API credentials. These values are encrypted at rest.</p>
        </div>
        <ConfigurationStatus status={configStatus === "configured" ? "complete-locally" : configStatus === "has-errors" ? "has-errors" : "not-configured"} />
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-xl">
          <label htmlFor="tt-client-key" className="block text-[12px] font-medium text-slate-700">
            Client Key
          </label>
          <input
            id="tt-client-key"
          ...
          />
          {errors.client_key && <p className="mt-1 text-[11px] text-rose-600">{errors.client_key}</p>}
        </div>
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-xl">
          <label htmlFor="tt-env" className="block text-[12px] font-medium text-slate-700">
            Environment
          </label>
          <select
            id="tt-env"
      ...
      <div className="mt-2 flex items-center justify-end gap-3 border-t border-white/10 pt-4">
        <button
          ...
          onClick={handleSave}
          disabled={isSaving}
          className="inline-flex h-10 min-h-[44px] items-center gap-2 rounded-xl bg-blue-600 px-4 text-[13px] font-medium text-white shadow-[0_8px_24px_rgba(37,99,235,0.25)] transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
